...
单词，以及为值单词F、N、P、T和S设置的负值。*/
  ...
  uint8_t int_value = 0;
  uint16_t mantissa = 0;
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) { char_counter = 3; } //在“$J”之后开始解析=`
  else { char_counter = 0; }

  while (line[char_counter] != 0) { //循环，直到行中不再有g代码字。

    //导入下一个g代码字，应为字母后跟值。否则，就会出错。
    letter = line[char_counter];
    if((letter < 'A') || (letter > 'Z')) { FAIL(STATUS_EXPECTED_COMMAND_LETTER); } // [期望词字母]
    char_counter++;
    if (!read_float(line, &char_counter, &value)) { FAIL(STATUS_BAD_NUMBER_FORMAT); } // [期望词值]
    ...
    switch(letter) {
      ...
      case 'G':
        ...
        switch(int_value) {
          case 10: case 28: case 30: case 92:
            ...
            if (mantissa == 0) { // 忽略 G28.1, G30.1, and G92.1
          ...
          case 90: case 91:
            if (mantissa == 0) {
              ...
              if ((mantissa != 10) || (int_value == 90)) { FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [G90.1 不支持]
              mantissa = 0; //设置为零表示有效的非整数G命令。
          ...
          case 43: case 49:
            ...
            } else { FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } //[不支持的G43.x命令]
            mantissa = 0; //设置为零表示有效的非整数G命令。
            break;
          ...
          case 61:
            ...
            if (mantissa != 0) { FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [G61.1 不支持]
            // gc_block.modal.control = CONTROL_MODE_EXACT_PATH; // G61
            break;
          default: FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); //[不支持的G命令]
        }
        if (mantissa > 0) { FAIL(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER); } //[不支持或无效的Gxx.x命令]
      ...
      case 'M':
        ...
        //确定“M”命令及其模式组
        if (mantissa > 0) { FAIL(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER); } //[没有Mxx.x命令]
        switch(int_value) {
          case 0: case 1: case 2: case 30:
            word_bit = MODAL_GROUP_M4;
            switch(int_value) {
              case 0: gc_block.modal.program_flow = PROGRAM_FLOW_PAUSED; break; //程序暂停
              case 1: break; //不支持可选停止。忽视
              default: gc_block.modal.program_flow = int_value; //程序结束和重置
            }
            break;
          case 3: case 4: case 5:
            ...
            switch(int_value) {
              ...
              case 9: gc_block.modal.coolant = COOLANT_DISABLE; break; //M9同时禁用M7和M8。
            }
            break;
            ...
            case 56:
              ...
              break;
          #endif
          default: FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); //[不支持的M命令]
        }
...
//所有当前坐标系和G92偏移。
  switch (gc_block.non_modal_command) {
    case NON_MODAL_SET_COORDINATE_DATA:
      ...
      if (bit_isfalse(value_words,((1<<WORD_P)|(1<<WORD_L)))) { FAIL(STATUS_GCODE_VALUE_WORD_MISSING); } //[P/L缺字]
      ...
      else { coord_select = gc_block.modal.coord_select; } //指数P0作为活动坐标系
      
      //注意：将参数数据存储在IJK值中。根据规则，它们不与此命令一起使用。
      if (!settings_read_coord_data(coord_select,gc_block.values.ijk)) { FAIL(STATUS_SETTING_READ_FAIL); } //[EEPROM读取失败]
...
//注：G28/30中的G53从未激活，因为它们位于同一模态组中。
  ...
  if (gc_block.modal.motion == MOTION_MODE_NONE) {
    ...
    if (axis_words) { FAIL(STATUS_GCODE_AXIS_WORDS_EXIST); } //[不允许使用轴字]
...
//该值必须为正。在反时限模式下，每个块必须传递一个正值。
      ...
      if (gc_block.values.f == 0.0) { FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); } //[进给速度未定义]
...
//如果圆圈为逆时针方向，则反转h_x2_div_d的符号（见下图）
            ...
            */
            ...
            if (delta_r > 0.005) {
              if (delta_r > 0.5) { FAIL(STATUS_GCODE_INVALID_TARGET); } //[电弧定义误差]>0.5mm
              if (delta_r > (0.001*gc_block.values.r)) { FAIL(STATUS_GCODE_INVALID_TARGET); } //[电弧定义误差]>0.005mm和0.1%半径
            }
          }
          break;
...
//当点动完成或取消时，在协议_execute_realtime（）中更新点动后的最终解析器位置。
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
...
//注意：如果没有行号，则该值为零。
  ...
  if (gc_state.modal.feed_rate) { pl_data->condition |= PL_COND_FLAG_INVERSE_TIME; } //设置用于规划器使用的条件标志。
